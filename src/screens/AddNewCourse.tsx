...
const AddNewCourse: FC<any> = ({navigation}) => {
  ...
  useEffect(() => {
    if (docRef === null) {
      ...
      const documentRef = firestore().collection('Courses').doc(randomUUID);
  ...
  const onAttachFilesPress = useCallback(async () => {
    const tmpFiles = (await pickFiles(false)) ?? [];
      ...
      await docRef?.set(
        {
          ...
          description: message,
          createdBy: 'Siva',
          imgUrl: `gs://course-management-system-ff6d0.appspot.com/${
            files[0]?.filename ?? ''
          }`,
        },
        {merge: true},
  ...
  }, [docRef, title, message, navigation, files]);
    ...
    <SafeAreaView style={styles.mainView}>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <Pressable onPress={onAttachFilesPress} style={styles.attachFilesBtn}>
          <Heading.SemiBold size={12}>Upload Wall Picture</Heading.SemiBold>
        </Pressable>
